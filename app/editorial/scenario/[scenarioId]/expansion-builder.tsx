...
export function ExpansionScenarioBuilder({
  scenarioId,
  rules,
  catalog,
}: {
  scenarioId: string;
  rules: ExpansionRuleRow[];
  catalog: CatalogRow[];
}) {
  ...
  const [enabled, setEnabled] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(rules.map((rule) => [rule.role, rule.preset_status !== "optional"])),
  );
  ...
  const pools = useMemo(
    () => Object.fromEntries(rules.map((rule) => [rule.role, roleCandidates(rule, catalog)])) as Record<string, CatalogRow[]>,
    [rules, catalog],
  );

  const selectedRows = rules.map((rule) => ({
    rule,
    product: pools[rule.role]?.find((row) => row.offer_id === selected[rule.role]),
    enabled: enabled[rule.role] !== false,
  }));

  const quantityFor = (rule: ExpansionRuleRow) =>
    rule.quantity_rule === "per_guest" ? guests : Math.max(1, manualQuantity[rule.role] || 1);
      ...
      <div className={builderStyles.expansionBuilderMain}>
        {guestOptions.length > 1 && (
          <div className={builderStyles.expansionGuests}>
            ...
            <p>Автоматически пересчитываются только позиции с quantity_rule = per_guest.</p>
          </div>
        )}
                      ...
                      <label className={builderStyles.expansionSelectLabel}>
                        ...
                        <select value={selected[rule.role] || ""} onChange={(event) => setSelected((current) => ({ ...current, [rule.role]: event.target.value }))}>
                          ...
                          {candidates.map((candidate) => {
                            const candidatePrice = toPrice(candidate.price);
                            const unavailable = isUnavailable(candidate.availability_status);
                            return (
                              <option key={candidate.offer_id} value={candidate.offer_id} disabled={unavailable}>
                                {candidate.product_name} · {candidate.collection}{candidate.size ? ` · ${candidate.size}` : ""} · {candidatePrice ? formatRub(candidatePrice) : "Цена уточняется"}{unavailable ? " · нет в наличии" : ""}
                              </option>
                            );
                          })}
                        </select>
                          ...
                          <div className={builderStyles.expansionSelectedCopy}>
                            ...
                            <div className={builderStyles.expansionQuantity}>
                              ...
                              {rule.quantity_rule === "per_guest" ? (
                                ...
                                <div>
                                  <button type="button" onClick={() => setManualQuantity((current) => ({ ...current, [rule.role]: Math.max(1, quantity - 1) }))} aria-label="Уменьшить количество">−</button>
                                  <b>{quantity}</b>
                                  <button type="button" onClick={() => setManualQuantity((current) => ({ ...current, [rule.role]: quantity + 1 }))} aria-label="Увеличить количество">+</button>
                                </div>
                              )}
                            </div>
                          </div>
        ...
        <div className={builderStyles.expansionSummaryList}>
          {activeRows.map(({ rule, product }) => (
            <div key={rule.role}>
              ...
              <b>{quantityFor(rule)} × {toPrice(product!.price) ? formatRub(toPrice(product!.price)!) : "Цена уточняется"}</b>
            </div>
          ))}
        </div>
