...
function solutionImage(solution: TableSolution, rows: CatalogRow[]) {
  return solution.heroImage || (solution.previewFile ? `/assets/images/constructor/${solution.previewFile}` : rows[0]?.primary_image_url) || "/assets/images/image-placeholder.svg";
}
...
function cartItemFromRow(row: CatalogRow, quantity = 1) {
  ...
  return {
    ...
    skus: [{ id: `ready-${row.offer_id || id}`, article: row.vendor_code || String(row.offer_id), productId: id, color, colorHex: swatchColor(color), size, material: row.material || "", composition: row.material || "", price, image: images[0] || "/assets/images/image-placeholder.svg", gallery: images.slice(1) }],
  };
}
    ...
    <main className="rs57-landing">
      ...
      <section className="rs57-index" aria-labelledby="rs57-index-title">
        ...
        {visible.length ? <div className="rs57-solution-grid">{visible.map(({ solution, rows, groups, guestOptions, image, from }) => <article className="rs57-solution-card" key={solution.id}>
          <Link className="rs57-solution-media" href={`/ready-solutions/${solution.id}/`}><RemoteImage src={image} fallbackSrc="/assets/images/image-placeholder.svg" alt={solution.name}/></Link>
  ...
  return <div className={`rs57-page rs57-wizard-page rs66-zara rs68-wizard rs68-step-${step}`}>
    ...
    <main className="rs57-wizard-shell">
      ...
      <section className="rs57-wizard-hero">
        <div className="rs57-wizard-hero-media"><RemoteImage src={solutionImage(solution, rows)} fallbackSrc="/assets/images/image-placeholder.svg" alt={solution.name}/></div>
...
function ReadyCatalogCard({ option, selected, color, size, quantity, guests, replacing, replacingSelf, onToggle, onColor, onSize, onQty }: { option: SolutionProductOption; selected: boolean; color: string; size: string; quantity: number; guests: number; replacing: boolean; replacingSelf: boolean; onToggle: () => void; onColor: (value: string) => void; onSize: (value: string) => void; onQty: (value: number) => void }) {
  ...
  return <div className={`rs57-product-wrap ${selected ? "is-selected" : ""} ${replacingSelf ? "is-replacing" : ""}`}>
    <article className="product-card rs57-product-card">
      ...
      {productHref ? <a className="product-image" href={productHref} target="_blank" rel="noreferrer"><RemoteImage src={image} fallbackSrc="/assets/images/image-placeholder.svg" alt={option.title}/></a> : <div className="product-image"><RemoteImage src={image} fallbackSrc="/assets/images/image-placeholder.svg" alt={option.title}/></div>}
