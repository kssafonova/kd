...
function cartItemFromRow(row: CatalogRow, quantity:number) {
  ...
  return { id,name:displayProductName(row.product_name),note:[displayCollectionName(sourceCollectionForRow(row)),row.material].filter(Boolean).join(" · "),price,image:images[0]||"/assets/images/image-placeholder.svg",gallery:images.slice(1),selectedColor:color,selectedSize:size,selectedSkuId:`ready71-${row.offer_id||id}`,quantity,skus:[{id:`ready71-${row.offer_id||id}`,article:row.vendor_code||String(row.offer_id),productId:id,color,colorHex:swatchColor(color),size,material:row.material||"",composition:row.material||"",price,image:images[0]||"/assets/images/image-placeholder.svg",gallery:images.slice(1)}] };
}
...
function ProductCard({option,selected,color,size,quantity,guests,onToggle,onColor,onSize,onQty}:{option:SolutionProductOption;selected:boolean;color:string;size:string;quantity:number;guests:number;onToggle:()=>void;onColor:(v:string)=>void;onSize:(v:string)=>void;onQty:(v:number)=>void}) {
  const colors=optionColors(option), sizes=optionSizes(option,color), row=pickOptionVariant(option,color,size), image=rowImages(row)[0]||"/assets/images/image-placeholder.svg";
  ...
  return <article className={`rs71-product ${selected?"is-selected":""}`}>
    <div className="rs71-product-media"><RemoteImage src={image} fallbackSrc="/assets/images/image-placeholder.svg" alt={displayProductName(option.title)}/><label className="rs71-check"><input type="checkbox" checked={selected} onChange={onToggle}/><i>✓</i></label></div>
...
export function ReadySolutionWizard({scenarioId}:{scenarioId:string}) {
  ...
  return <div className="rs71-page"><Header/><main className="rs71-shell">
    ...
    {step===1&&<section className="rs71-parameters"><header><small>ПАРАМЕТРЫ</small><h2>Настройте основу</h2><p>Количество персон и коллекции определяют состав решения.</p></header><div className="rs71-param-block"><h3>Количество персон</h3><nav className="rs71-person-rail">{guestOptions.map((v)=><button key={v} className={guests===v?"is-active":""} onClick={()=>setGuests(v)}>{v}</button>)}</nav></div><div className="rs71-param-block"><h3>Коллекции в решении</h3><div className="rs71-collection-grid">{activeCollections.map((name)=>{const row=catalog.catalog.find((r)=>norm(sourceCollectionForRow(r))===norm(name)); return <button type="button" key={name} className="is-active" onClick={()=>toggleCollection(name)}><RemoteImage src={rowImages(row)[0]||solution.heroImage||baseRows[0]?.primary_image_url||"/assets/images/caps_led.png"} fallbackSrc={solution.heroImage||baseRows[0]?.primary_image_url||"/assets/images/caps_led.png"} alt={name}/><i>✓</i><span>{displayCollectionName(name)}</span></button>;})}</div>{extraChoices.length>0&&<><h4>Добавить коллекцию</h4><div className="rs71-collection-grid is-extra">{extraChoices.map((name)=>{const row=catalog.catalog.find((r)=>norm(sourceCollectionForRow(r))===norm(name)); const active=activeCollections.some((c)=>norm(c)===norm(name)); return <button type="button" key={name} className={active?"is-active":""} onClick={()=>toggleCollection(name)}><RemoteImage src={rowImages(row)[0]||solution.heroImage||baseRows[0]?.primary_image_url||"/assets/images/caps_led.png"} fallbackSrc={solution.heroImage||baseRows[0]?.primary_image_url||"/assets/images/caps_led.png"} alt={name}/><i>{active?"✓":"+"}</i><span>{displayCollectionName(name)}</span></button>;})}</div></>}</div></section>}
    ...
    {step===3&&<section className="rs71-result"><header><small>ВАШЕ РЕШЕНИЕ</small><h1>{solution.name}</h1><p>{selectedRows.length} предметов · {guests} персон</p></header><div className="rs71-moodboard">{selectedRows.map(({row,option},index)=><div key={`${option.id}-${row.offer_id}`} className={`rs71-mood-${(index%7)+1}`}><RemoteImage src={rowImages(row)[0]||"/assets/images/image-placeholder.svg"} fallbackSrc="/assets/images/image-placeholder.svg" alt={displayProductName(option.title)}/></div>)}</div><div className="rs71-result-groups">{groups.map((group)=>{const items=selectedRows.filter((item)=>item.group.id===group.id); if(!items.length)return null; return <details key={group.id}><summary><span>{group.title}</span><small>{items.length}</small><b>+</b></summary><div>{items.map(({row,option,quantity})=><article key={`${option.id}-${row.offer_id}`}><RemoteImage src={rowImages(row)[0]||"/assets/images/image-placeholder.svg"} fallbackSrc="/assets/images/image-placeholder.svg" alt={displayProductName(option.title)}/><div><h3>{displayProductName(option.title)}</h3><p>{[displayCollectionName(sourceCollectionForRow(row)||option.collection||""),row.color,row.size||row.volume].filter(Boolean).join(" · ")}</p><span>{quantity} × {money(priceOf(row))}</span></div><button type="button" onClick={()=>{setActiveGroup(group.id);setStep(2);}}>Заменить</button><button type="button" onClick={()=>setSelected((c)=>({...c,[option.id]:false}))}>Удалить</button></article>)}</div></details>;})}</div><button type="button" className="rs71-edit" onClick={()=>setStep(2)}>Изменить состав</button></section>}
  </main>
