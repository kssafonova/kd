...
export function ReadySolutionWizard({scenarioId}:{scenarioId:string}) {
  ...
  return <div className="rs71-page"><Header/><main className="rs71-shell">
    ...
    {step===1&&<section className="rs71-parameters"><header><small>ПАРАМЕТРЫ</small><h2>Настройте основу</h2><p>Количество персон и коллекции определяют состав решения.</p></header><div className="rs71-param-block"><h3>Количество персон</h3><nav className="rs71-person-rail">{guestOptions.map((v)=><button key={v} className={guests===v?"is-active":""} onClick={()=>setGuests(v)}>{v}</button>)}</nav></div><div className="rs71-param-block"><h3>Коллекции в решении</h3><div className="rs71-collection-grid">{activeCollections.map((name)=>{const row=catalog.catalog.find((r)=>norm(sourceCollectionForRow(r))===norm(name)); return <button type="button" key={name} className="is-active" onClick={()=>toggleCollection(name)}><RemoteImage src={rowImages(row)[0]||solution.heroImage||baseRows[0]?.primary_image_url||"/images/editorial/caps_led.png"} fallbackSrc={solution.heroImage||baseRows[0]?.primary_image_url||"/images/editorial/caps_led.png"} alt={name}/><i>✓</i><span>{displayCollectionName(name)}</span></button>;})}</div>{extraChoices.length>0&&<><h4>Добавить коллекцию</h4><div className="rs71-collection-grid is-extra">{extraChoices.map((name)=>{const row=catalog.catalog.find((r)=>norm(sourceCollectionForRow(r))===norm(name)); const active=activeCollections.some((c)=>norm(c)===norm(name)); return <button type="button" key={name} className={active?"is-active":""} onClick={()=>toggleCollection(name)}><RemoteImage src={rowImages(row)[0]||solution.heroImage||baseRows[0]?.primary_image_url||"/images/editorial/caps_led.png"} fallbackSrc={solution.heroImage||baseRows[0]?.primary_image_url||"/images/editorial/caps_led.png"} alt={name}/><i>{active?"✓":"+"}</i><span>{displayCollectionName(name)}</span></button>;})}</div></>}</div></section>}
