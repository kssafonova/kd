...
export default function Home() {
  ...
  return (
    <main className={`view-${view}`}>
      ...
      {view === "collections" && <CollectionsView openEditorial={(item)=>{setEditorial(item);go("editorial")}} onProduct={openProduct} onQuick={setPlpSize} favorite={favorite} favorites={favorites} />}
...
function CollectionsView({ openEditorial,onProduct,onQuick,favorite,favorites }: { openEditorial:(editorial:Editorial)=>void; onProduct:(product:Product)=>void; onQuick:(product:Product)=>void; favorite:(id:number)=>void; favorites:number[] }) {
...
function ProductSizeRows({sizes,selectedSize,setSelectedSize,quantity,setQuantity,notify,unavailableLast=true,unavailableSizes=[],oldPrice}:{sizes:readonly (readonly [string,number])[];selectedSize:string;setSelectedSize:(size:string)=>void;quantity:number;setQuantity:(quantity:number)=>void;notify:(size:string)=>void;unavailableLast?:boolean;unavailableSizes?:string[];oldPrice?:number}){
  ...
  return <div className="sizes quantity-sizes">{sizes.map(([name,price],index)=>{
    ...
    return <div key={name} className={`size-row ${selectedSize===name&&!unavailable?"active":""} ${unavailable?"unavailable":""}`}>
      <button disabled={unavailable} onClick={()=>{if(!unavailable){setSelectedSize(name);setQuantity(1)}}}><span>{name}</span>{selectedSize!==name&&!unavailable&&<b><span>{priceKnown(price)?fmt(price):"Цена уточняется"}</span>{priceKnown(price)&&oldPrice&&oldPrice>price&&<del>{fmt(oldPrice)}</del>}</b>}</button>
