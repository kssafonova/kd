...
export default function Home() {
  ...
  return (
    <main className={`view-${view}`}>
      ...
      {view === "editorial" && <EditorialView editorial={editorial} selectProduct={openProduct} quickAdd={setPlpSize} favorite={favorite} favorites={favorites} />}
      {view === "product" && <ProductView product={selected} favorite={favorite} liked={favorites.includes(selected.id)} chooseSize={() => setSizeSheet(true)} add={(p) => add(p,p.selectedSize,p.quantity)} selectProduct={openProduct} quickAdd={setPlpSize} recentlyViewed={recentlyViewed} />}
...
function EditorialView({ editorial, selectProduct, quickAdd, favorite, favorites }: { editorial:Editorial; selectProduct:(product:Product)=>void; quickAdd:(product:Product)=>void; favorite:(id:number)=>void; favorites:number[] }) {
  ...
  return <div className={`editorial-page zara-editorial editorial-variant-${variant}`}>
    ...
    <section className="editorial-products zh-editorial-products">
      ...
      <div className="product-grid">{items.map(item=><ProductCard product={item} key={`${editorial.id}-${item.id}`} onClick={selectProduct} onQuick={quickAdd} favorite={favorite} liked={favorites.includes(item.id)}/>)}</div>
    </section>
  </div>;
}
...
].map(section=><section className={`pdp-accordion-item ${open===section.title?"open":""}`} key={section.title}><button className="pdp-accordion-trigger" onClick={()=>setOpen(open===section.title?"":section.title)} aria-expanded={open===section.title}><span>{section.title}</span><Icon name="chevron"/></button>{open===section.title&&<div className="pdp-accordion-panel">{section.content}</div>}</section>)}</div></div></div>{product.hasRichContent&&<RichContent product={product} selectProduct={selectProduct}/>}<ProductRecommendations product={product} selectProduct={selectProduct} quickAdd={quickAdd} favorite={favorite} recentlyViewed={recentlyViewed}/>{storesOpen&&<BoutiqueMap close={()=>setStoresOpen(false)}/>}</div>;
}
...
function ProductRecommendations({product,selectProduct,quickAdd,favorite,recentlyViewed}:{product:Product;selectProduct:(product:Product)=>void;quickAdd:(product:Product)=>void;favorite:(id:number)=>void;recentlyViewed:number[]}){
  ...
  const viewedProducts=recentlyViewed
    ...
    .slice(0,4);
  return <>
    <section className="post-rich-recommendations category-recommendations"><div className="section-head"><p>ПРОДОЛЖИТЬ ВЫБОР</p><h2>Товары из этой категории</h2></div><ProductRail className="recommendation-product-rail" items={categoryProducts} onProduct={selectProduct} onQuick={quickAdd} favorite={favorite} favorites={[]}/></section>
    {viewedProducts.length>0&&<section className="post-rich-recommendations recently-viewed-recommendations" style={{marginTop:0,paddingTop:42}}><div className="section-head"><p>ИСТОРИЯ ПРОСМОТРОВ</p><h2>Вы недавно смотрели</h2></div><ProductRail className="recommendation-product-rail" items={viewedProducts} onProduct={selectProduct} onQuick={quickAdd} favorite={favorite} favorites={[]}/></section>}
  </>;
}
