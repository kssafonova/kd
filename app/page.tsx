...
function ProductCard({ product, onClick, onQuick, favorite, liked, selectionMode=false, selected=false, onSelect }: { product:Product; onClick:(p:Product)=>void; onQuick:(p:Product)=>void; favorite:(n:number)=>void; liked:boolean; selectionMode?:boolean; selected?:boolean; onSelect?:()=>void }) {
...
function LunaEditorialView({ editorial, selectProduct, favorite, favorites, quickAdd, addToCart }: { editorial:Editorial; selectProduct:(product:Product)=>void; favorite:(id:number)=>void; favorites:number[]; quickAdd:(product:Product)=>void; addToCart:(product:Product)=>void }) {
  ...
  const startStoryPurchase=()=>{if(!active)return;setSelectedIds(active.productIds);setSizes(current=>{const next={...current};active.productIds.forEach(id=>delete next[id]);return next});setStoryBuying(true)};
  const closeStory=()=>{setStoryBuying(false);setStory(null)};
  const addStory=()=>{const chosen=selectedIds.map(itemById).filter(Boolean).map(p=>configured(p!));if(!chosen.length||!storyReady)return;setStoryBuying(false);setStory(null);chosen.forEach(addToCart)};
  ...
  return <div className="luna-clean-page">
    ...
    {active&&<div className="luna-clean-overlay" role="dialog" aria-modal="true">
      ...
      <section className="luna-clean-story">
        ...
        <div className={`luna-clean-products ${storyBuying?"is-buying":""}`}>{storyProducts.map(item=>{const selected=selectedIds.includes(item.id);const color=colorById[item.id]??item.selectedColor;const options=getProductSizeOptions(item,color);const size=sizes[item.id]??(options.length===1?(options[0]?.[0]??item.selectedSize??""):"");const quantity=qty[item.id]??1;return <div className={`luna-story-card-shell ${storyBuying?(selected?"selected":"not-selected"):""}`} key={`${active.id}-${item.id}`}>
          <ProductCard product={item} onClick={quickAdd} onQuick={quickAdd} favorite={favorite} liked={favorites.includes(item.id)} selectionMode={storyBuying} selected={selected} onSelect={()=>toggle(item.id)}/>
