          ...
          {selectedRows.length ? <>
            ...
            <section className="rs59-moodboard" aria-labelledby="rs59-moodboard-title">
              ...
              <div className="rs59-moodboard-grid">
                {selectedRows.slice(0, 4).map(({ row, option, group }, index) => <article className={`rs59-moodboard-tile rs59-tile-${index}`} key={`mood-${option.id}`}><RemoteImage src={rowImages(row)[0] || "/assets/images/image-placeholder.svg"} fallbackSrc="/assets/images/image-placeholder.svg" alt={option.title}/><div className="rs59-tile-overlay"><span><small>{group.title}</small><strong>{option.title}</strong></span><div><button type="button" onClick={() => startReplace(option, group)}>Заменить</button><button type="button" onClick={() => removeItem(option.id)}>Удалить</button></div></div></article>)}
                <figure className="rs59-moodboard-tile rs59-moodboard-scene"><RemoteImage src={solutionImage(solution, rows)} fallbackSrc="/assets/images/image-placeholder.svg" alt={`${solution.name} — интерьер`}/><figcaption><small>{solution.space}</small><strong>{solution.name}</strong></figcaption></figure>
                {selectedRows.slice(4, 8).map(({ row, option, group }, index) => <article className={`rs59-moodboard-tile rs59-tile-${index + 5}`} key={`mood-${option.id}`}><RemoteImage src={rowImages(row)[0] || "/assets/images/image-placeholder.svg"} fallbackSrc="/assets/images/image-placeholder.svg" alt={option.title}/><div className="rs59-tile-overlay"><span><small>{group.title}</small><strong>{option.title}</strong></span><div><button type="button" onClick={() => startReplace(option, group)}>Заменить</button><button type="button" onClick={() => removeItem(option.id)}>Удалить</button></div></div></article>)}
